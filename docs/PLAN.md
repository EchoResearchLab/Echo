...
- **会话过期不再是死路**：服务端对过期会话返回 401「请先登录」，前端此前把它当普通错误字符串内联渲染——用户停在一个每块面板都写着"请先登录"的工作台上，按钮还在、点了还是 401，只能自己想到去刷新。现在 `api::decode` 与 SSE 流在 401 时通知根组件重新鉴权，直接回登录页；`/api/auth/*` 排除在外（登录密码错也是 401，拿它回登录页既没意义又会冲掉刚渲染的错误提示），这条排除同时保证不会自我循环。并发请求同时撞 401 只通知一次，任何成功响应复位。
  实测依据：开启鉴权后受保护接口确实返回 401，且 `/api/auth/me` 在无会话时返回 `200 {user:null}`——所以重新鉴权必然落到登录页。浏览器内"已挂载工作台 → 401 → 登录页"这一跳未做实跳验证，它需要一个已登录会话。
- **首屏不再白屏**：`<body>` 原来是空的，CSS 到位到 WASM 挂载之间（release WASM 1.62 MB，慢网上数秒）用户看到的是一整屏纯白。index.html 内置静态首屏骨架，复用令牌化的 `.boot-screen`（自带 160ms 延迟淡入，快网上一帧都不露出），挂载前由 `start()` 移除。
- **深色不再闪白**：index.html 把 `data-theme="light"` 写死在 `<html>` 上，而令牌层的"跟随系统"靠**没有**该属性生效（`theme.rs` 白纸黑字如此设计）——系统深色用户必定先吃一整屏白底直到 WASM 起来。去掉写死值；`theme-color` 同时补上明暗两套，并对齐真正的画布色。
- **标签页标题跟随上下文**：标题此前恒为品牌名。研究天然多开标签页，三个页签长得一模一样等于没有标签栏。现在研究页用**当轮对话的第一个问题**（不是股票代码——"苹果的盈利质量正在变化吗?" 比 "AAPL" 更好认），资料库/设置用切面名，登录页用登录/注册。
- **未做且有理由**：不加"复制研究链接"。深链 `/research/:id` 本身是通的，但私有会话受 RLS 保护，链接发给同事只会 404——做一个看起来能分享、实际不能分享的按钮比没有更糟。`assets/og.png` 是 1.69 MB / 1536×1024（OG 标准 1200×630），偏大且尺寸不对，但那是品牌图，重新导出属于设计侧决定，此处只登记不擅动。
